/*
 * MODIFIED for Anytype Marks (see [PATCH Anytype Marks] markers):
 * two innerHTML assignments replaced by equivalent DOM operations
 * (node-clone cache restore; createContextualFragment for <noscript>
 * content) to satisfy AMO review. No behavioral change intended.
 * Original: https://github.com/mozilla/readability v0.6.0
 *
...
Readability.prototype = {
  ...
  _grabArticle(page) {
    ...
    // [PATCH Anytype Marks] Cache de nœuds clonés au lieu d'une chaîne
    // HTML : évite l'affectation innerHTML lors de la restauration
    // (exigence de revue AMO), comportement identique.
    var pageCacheClone = page.cloneNode(true);
    ...
    while (true) {
      ...
      if (textLength < this._charThreshold) {
        ...
        // [PATCH Anytype Marks] Restauration par clonage de nœuds.
        while (page.firstChild) {
          page.removeChild(page.firstChild);
        }
        var pageRestored = pageCacheClone.cloneNode(true);
        while (pageRestored.firstChild) {
          page.appendChild(pageRestored.firstChild);
        }
  ...
  _unwrapNoscriptImages(doc) {
    ...
    this._forEachNode(noscripts, function (noscript) {
      ...
      // [PATCH Anytype Marks] DOMParser (document inerte, aucun script
      // exécuté) remplace l'affectation innerHTML (exigence de revue AMO) ;
      // les nœuds sont importés dans le document de travail.
      var noscriptDoc = new DOMParser().parseFromString(
        noscript.innerHTML,
        "text/html"
      );
      var noscriptBody = noscriptDoc.body;
      while (noscriptBody.firstChild) {
        tmp.appendChild(doc.importNode(noscriptBody.firstChild, true));
        noscriptBody.removeChild(noscriptBody.firstChild);
      }
